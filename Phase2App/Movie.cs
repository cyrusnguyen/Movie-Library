...
public class Movie : IMovie
{
    ...
    public bool AddBorrower(IMember member)
    {
        ...
        {
            if (Borrowers.Search(member) == false) //Using search method from previous assignment
            {
                Borrowers.Add(member); //Using Add method from previous assignment
                AvailableCopies--; //Decrement the number of avaliable copies
                NoBorrowings++; //Increment the number of borrowings taken on this movie
                return true;
            }
            else
            {
                Console.WriteLine("The member has already borrowed the DVD. Please return it before borrowing a new one.");
                return false; //Return false if member is not in movie borrower list
            }
        }
        else
        {
            Console.WriteLine("There's not enough DVDs in the library.");
            return false; //Return false if the the number of avaliable copies is below 1
        }
    }
    ...
    {
        //To be completed
        if (Borrowers.Search(member) == true) //Using search method from previous assignment
        {
            Borrowers.Delete(member); //Using delete method from previous assignment
            AvailableCopies++; //Increment the number of avaliable copies
            return true;
        }
        else
        {
            return false; //If there is no such member return false
        }
    ...
    //Highly inspired by the provided CompareTo code in the Phase 1 Assessment 
    public int CompareTo(IMovie another)
        ...
        {
            return -1; //If the title of the movie less than the other movie's dictionary order
        }
        else
            if (this.Title.CompareTo(other.Title) == 0)
        {
            return 0; //If this movie's title is equal to the other movie's title by dictionary order
        }
        else
            return 1; //If the movie's titel is greater than the other movie's title by dictionary order 
    }
